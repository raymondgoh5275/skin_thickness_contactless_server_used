// -----------------------------------------------------------------------
// <copyright file="Config.cs" company="ATS-Global">
// Copyright (c) 2012 ATS Ltd.
// </copyright>
// -----------------------------------------------------------------------

namespace ATS_Global.INI
{
    using System;
    ...
    public class Config
    {
        ...
        public SettingsGroup this[string index]
        {
            get
            {
                if (this.SettingGroups.ContainsKey(index))
            ...
            set { this.SettingGroups[index] = value; }
        }
        ...
        public void Load(Stream stream)
        {
            ...
            using (StreamReader reader = new StreamReader(stream))
            {
                while (!reader.EndOfStream)
                {
                    ...
                    //found group name
                    if ((line.Trim().StartsWith("[")) && (line.Trim().EndsWith("]")))//match.Success)
                    {
                        ...
                        if (line.Length == 2)//match.Value.Length == 2)
                            throw new Exception(string.Format("Group must have name (line {0})", lineNumber));

                        //set our current group information
                        currentGroupName = line.Substring(1, line.Length - 2); //match.Value.Substring(1, match.Length - 2);
